...
from skillpod.cli.commands import profile_apply
...
# ---------------------------------------------------------------------------
# CLI command: preview vs --yes
# ---------------------------------------------------------------------------


def test_cli_preview_does_not_change_state(tmp_path: Path) -> None:
    repo, _sha = make_root_skill_repo(tmp_path / "src", repo_name="audit")
    home = tmp_path / "home"
    _write_global_profile(
    ...
    profile_apply.run(
        "dev", project_root=tmp_path, yes=False, json_output=False, home=home
    )
    # Preview must not download or link anything.
    assert not global_skill_dir("audit", home).exists()
    assert not global_agent_skill_dir("claude", "audit", home).exists()


def test_cli_apply_yes_materialises_and_sets_active(tmp_path: Path) -> None:
    repo, _sha = make_root_skill_repo(tmp_path / "src", repo_name="audit")
    home = tmp_path / "home"
    _write_global_profile(
        home, "dev", {"agents": ["claude"], "skills": [{"name": "audit", "source": str(repo)}]}
    ...
    profile_apply.run(
        "dev", project_root=tmp_path, yes=True, json_output=False, home=home
    )
    assert is_managed_global_fanout(
        global_agent_skill_dir("claude", "audit", home), "audit", home
    )
...
def test_cli_unknown_profile_exits_nonzero(tmp_path: Path) -> None:
    with pytest.raises(typer.Exit) as exc_info:
        profile_apply.run(
            "nope", project_root=tmp_path, yes=False, json_output=False, home=tmp_path
        )
